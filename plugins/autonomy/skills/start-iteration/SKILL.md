...
| 3. Load older context | Read summary.md if >5 iterations (read-only — do NOT update) | Read |
...
If more than 5 iterations exist, **read** the existing summary.md for older context. Do NOT update or regenerate it here.

1. **Read summary.md if it exists:**
   ```
   file: "autonomy/[goal-name]/summary.md"
   ```
   - If present: incorporate into the state presented to the user.
   - If absent or stale: proceed without it. It will be (re)generated at the end of the next milestone iteration (5, 10, 15, …).

**DO NOT dispatch journal-summarizer at the start of an iteration.** summary.md is owned by the `end-iteration` skill and is only updated at the end of every 5th iteration (5, 10, 15, 20, …). Updating it here causes redundant rewrites, wastes tokens, and risks summarizing iterations that have not concluded yet.

| Rationalization at iteration start | Reality |
|------------------------------------|---------|
| "Summary looks outdated, I'll refresh it" | NO. Wait for the next iteration-end milestone. |
| "There's no summary yet, I'll create one" | NO. It will be created at the end of iteration 5/10/15/etc. |
| "I'll just have the agent regenerate it real quick" | NO. Read-only at iteration start. |
...
| "I'll create summary.md myself" | NO. summary.md is owned by `end-iteration` and only updated at the end of every 5th iteration. |
| "summary.md is missing/stale, I'll refresh it now" | NO. At iteration start it is **read-only**. Wait for the next end-of-iteration milestone (5, 10, 15, …). |
| "I'll dispatch journal-summarizer at the start" | NO. Never dispatch journal-summarizer from `start-iteration`. It is only dispatched by `end-iteration` on milestone iterations. |
